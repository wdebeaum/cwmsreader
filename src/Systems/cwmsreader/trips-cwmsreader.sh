#!/bin/sh
#
# File: trips-cwmsreader.sh
# Creator: Lucian Galescu, based on template by George Ferguson
# Created: Wed Jun 20 10:38:13 2012
# Time-stamp: <Mon Jun 22 18:51:08 CDT 2020 lgalescu>
#
# trips-cwmsreader: Run TRIPS/CWMSREADER
#
...
echo 'This is TRIPS/CWMSREADER version 0'
...
usage="trips-$TRIPS_SYSNAME [-debug] [-port 6200] [-display tty] [-nouser] [-nochat] [-nolisp] [-nocsm] [-graphviz-display true] [-mapper] [-data DIR] [-store DIR]"
...
if test -z "$nolisp"; then
  if test -n "$mapper"; then
    export TRIPS_PARAMS_LISP=$TRIPS_BASE/etc/trips-$TRIPS_SYSNAME/params-cwmsmapper.lisp
  else
    export TRIPS_PARAMS_LISP=$TRIPS_BASE/etc/trips-$TRIPS_SYSNAME/params.lisp
  fi
